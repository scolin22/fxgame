...
void updateFruits(FruitCtrl *fruitCtrl) {
	...
	for (i = 0; i < NUM_PLAYERS*FRUITS_PER_PLAYER; i++) {
		if (fruitCtrl->fruits[i].timeLeft > 0) {
			if(fruitCtrl->fruits[i].velX != 0 || fruitCtrl->fruits[i].velY != 0)
	...
	if (fruit->velX > 0) {
		...
		if (fruit-> tossed > 1) {
			...
			if (checkThrowCollision(fruit)) {
				...
				changeTile(map, fruit->posX - TILE_SIZE, fruit->posY, lastTile);
			}
		}
		...
		if (fruit-> tossed > 1) {
			...
			if (checkThrowCollision(fruit)) {
				...
				changeTile(map, fruit->posX, fruit->posY - TILE_SIZE, lastTile);
			}
		}
		else if (checkFruitCollision(fruit)) {
			fruit->posY -= TILE_SIZE;
